...
export default function Descripcion() {
	...
	const onTouchEnd = () => {
		...
		if (isLeftSwipe || isRightSwipe) {
			if (isLeftSwipe) {
				setI((prev) => (prev + 1) % imagesMainPage.length)
			} else {
				setI(
					(prev) => (prev - 1 + imagesMainPage.length) % imagesMainPage.length
				)
			}
			resetTimer()
		}
			...
			<div className="mx-auto max-w-6xl px-4">
				...
				<div className="grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-10 items-start">
					...
					<div className="md:col-span-7 w-full h-full">
						<div
							className="relative w-full overflow-hidden rounded-2xl shadow-xl aspect-[4/3] md:aspect-auto md:h-full cursor-grab active:cursor-grabbing"
